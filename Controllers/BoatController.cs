    ...
    public class BoatController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (User.IsInRole(RoleName.BoatOwner))
            {
                ...
                // Filter results so that he only sees his boats rather than all
                // of them
            ...
            // Forbid access to the page if user is none of the roles of a boat
        ...
        public async Task<IActionResult> Details(int? id)
        {
            try
            {
                ...
                // If the user shouldn't be able to see that boat's information,
                // forbid the access
                return Forbid();
            }
        ...
        // POST: Boat/Create To protect from overposting attacks, enable the
        ...
        public async Task<IActionResult> Create([Bind("BoatId,Name,RegistrationNo,Width,Length,Depth,Type")] Boat boat)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    // Set the BoatOwner ID
                    if (User.IsInRole(RoleName.BoatOwner))
                    {
                        var person = await _userService.GetUserAsync(User);
                        boat.BoatOwnerId = _userService.GetBoatOwnerFromPerson(person).BoatOwnerId;
                    }
        ...
        // POST: Boat/Edit/5 To protect from overposting attacks, enable the
        // specific properties you want to bind to, for more details, see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
